...
It runs entirely in the browser and only uses YouTube’s existing caption data. It does not collect data, does not send data anywhere, and does not use external services. It works by intercepting YouTube’s caption response, extracting word-level timing data, grouping it into readable chunks based on timing and length rules, and rendering it in a synchronized overlay over the video. If YouTube changes its caption system, the extension may stop working until updated.

To install, open Chrome or any Chromium-based browser and go to the extensions page (chrome://extensions or equivalent), enable Developer Mode, click “Load unpacked,” and select the extension folder. Once installed, open a YouTube video with auto captions. A CC+ button will appear in the player controls to toggle the behavior.
